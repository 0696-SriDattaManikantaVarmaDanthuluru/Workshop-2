...
FROM customers
INNER JOIN orders ON customers.customer_id = orders.customer_id
GROUP BY customers.customer_id, customers.name order by customer_id ;
...
GROUP BY products.product_id, products.product_name order by product_id;
...
--9.. Retrieve Customers Who Have Placed More Than 1 Order
